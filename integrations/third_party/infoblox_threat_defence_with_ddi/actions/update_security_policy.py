...
def create_payload_security_policy(policy_name, description, block_dns_rebinding, safe_search, network_lists, dfps,
                                   roaming_device_groups, rules, tags, additional_params, existing_data):
    payload = {
        ...
        "block_dns_rebind_attack": existing_data.get(
            "block_dns_rebind_attack") if not block_dns_rebinding else block_dns_rebinding.lower() == "true",
        "safe_search": existing_data.get("safe_search") if not safe_search else safe_search.lower() == "true",
        "network_lists": get_nullable_field(network_lists, existing_data.get("network_lists"),
                                            parser=lambda v: parse_and_validate_int_list(v, "Network Lists")) or [],
        "dfps": get_nullable_field(dfps, existing_data.get("dfps"),
                                   parser=lambda v: parse_and_validate_int_list(v, "DFPS")) or [],
