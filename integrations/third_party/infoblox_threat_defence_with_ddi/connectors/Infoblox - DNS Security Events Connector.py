...
# events don't have any unique identifier from the API. Instead, events are tracked by timestamps (event_time). 
# Incremental fetching is handled by saving the latest event timestamp and using it as the
# start time for the next polling cycle, which naturally prevents duplicate processing.
# ==============================================================================
...
def validate_input_params(threat_level, limit, max_hours_backwards):
    ...
    Args:
        ...
        max_hours_backwards (str): Number of hours before the first connector iteration to retrieve alerts from for the first time
    ...
    if max_hours_backwards:
        max_hours_backwards = validate_integer_param(max_hours_backwards, "Max Hours Backwards",
                                                     zero_allowed=True, allow_negative=False)
...
def main(is_test_run):
    ...
    try:
        ...
        start_time = str(int(get_last_success_time(
            siemplify=siemplify,
        ...
        for dns_event in dns_events:
            ...
            try:
                ...
                siemplify.LOGGER.error(
                    f"Failed to process DNS Security Event. Error: {str(e)}"
                )
