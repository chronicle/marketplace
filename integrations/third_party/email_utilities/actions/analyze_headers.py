...
from ..core import email_parser_routing, email_utilities_manager
...
def ip_check(ip, domain):
    spf_record = email_utilities_manager.SpfRecord.from_domain(domain).record.split(" ")
    ...
    for include_domain in includes:
        include_spf = email_utilities_manager.SpfRecord.from_domain(
            include_domain,
        ).record.split(" ")
        includes.extend(
...
def parseHops(received):
    ...
    for hop in reversed(received):
        ...
        try:
            parsed_route = email_parser_routing.parserouting(hop)
        except Exception:
            raise
...
def buildResult(header, siemplify):
    ...
    dkim = email_utilities_manager.DKIM(logger=siemplify.LOGGER, headers=header)
    arc = email_utilities_manager.ARC(logger=siemplify.LOGGER, headers=header)
    ...
    try:
        ...
        for fromserver_str in reversed(header["received"]):
            if "by" in fromserver_str:
                fromserver = email_parser_routing.parserouting(fromserver_str)
                try:
                    if "by" in fromserver:
                    ...
                    if "by" in fromserver:
                        ...
                        try:
                            result["SourceServerIP"] = (
                                email_utilities_manager.Resolver().query(
                                    result["SourceServer"],
                                )[0][2]
                            )
                        except:
    ...
    try:
        result["StrongSPF"] = email_utilities_manager.SpfRecord.from_domain(
            result["FromDomain"],
        ).is_record_strong()
    except:
