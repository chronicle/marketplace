...
from . import email_parser_decode, email_parser_regex, email_parser_routing
...
class EmlParser:
    def __init__(
        ...
        if self.email_force_tld:
            email_parser_regex.email_regex = email_parser_regex.email_force_tld_regex
    ...
    def parse_email(self) -> dict:
        ...
        headers_struc["subject"] = email_parser_decode.decode_field(subject)
        ...
        try:
            ...
            _from = email_parser_decode.workaround_bug_27257(self.msg, "from")
        ...
        if msg_header_field != "":
            m = email_parser_regex.email_regex.search(msg_header_field)
            if m:
        ...
        if "date" in self.msg:
            try:
                ...
                headers_struc["date"] = email_parser_decode.robust_string2date(msg_date)
        ...
        try:
            ...
            for received_line in self.msg.get_all("received", []):
                ...
                parsed_routing = email_parser_routing.parserouting(received_line_flat)
                sending = {}
                sending["ips"] = []
                sending["domains"] = []
                sending["emails"] = []
                ...
                ips_in_received_line = email_parser_regex.ipv6_regex.findall(
                    received_line_flat,
                ) + email_parser_regex.ipv4_regex.findall(received_line_flat)
                ...
                for m in email_parser_regex.recv_dom_regex.findall(received_line_flat):
                    try:
                        ip_obj = ipaddress.ip_address(
                            m,
                        )  # type of findall is list[str], so this is correct
                ...
                for mail_candidate in email_parser_regex.email_regex.findall(
                    received_line_flat,
                ):
        ...
        for body_tup in raw_body:
            ...
            for body_slice in self.string_sliding_window_loop(body):
                ...
                for match in extractor.find_urls(body_slice):
                    # for match in email_parser_regex.email_regex.findall(body_slice):
                    c = URLExtract()
                ...
                # for match in email_parser_regex.dom_regex.findall(body_slice):
                for match in list_observed_urls:
                    try:
                        ...
                        pass
                for match in email_parser_regex.ipv4_regex.findall(body_slice):
                    try:
                ...
                for match in email_parser_regex.ipv6_regex.findall(body_slice):
                    try:
                        ipaddress_match = ipaddress.ip_address(match)
                    except ValueError:
                        continue
        ...
        for k in set(self.msg.keys()):
            ...
            try:
                ...
                decoded_values = (
                    email_parser_decode.workaround_field_value_parsing_errors(self.msg, k)
                )
        ...
        if body_length <= slice_step:
            ...
            for ptr_end in range(slice_step, body_length, slice_step):
                if " " in body[ptr_end - 1 : ptr_end]:
                    while not (
                        email_parser_regex.window_slice_regex.match(
    ...
    def get_uri_ondata(body: str) -> list[str]:
        ...
        for found_url in extractor.find_urls(body):
            # for found_url in email_parser_regex.url_regex_simple.findall(body):
            if "." not in found_url:
    ...
    def headeremail2list(self, header: str) -> list[str]:
        ...
        try:
            ...
            field = email.utils.getaddresses(
                email_parser_decode.workaround_bug_27257(self.msg, header),
            )
        ...
        for m in field:
            if not m[1] == "":
                if self.email_force_tld:
                    if email_parser_regex.email_force_tld_regex.match(m[1]):
        ...
        if msg.is_multipart():
            ...
            if (
                ...
                if charset is None:
                    ...
                    raw_body_str = email_parser_decode.decode_string(raw_body_str, None)
                else:
                    try:
    ...
    def prepare_multipart_part_attachment(
        ...
        if (
            ...
            if filename == "":
                ...
                filename = email_parser_decode.decode_field(filename)
