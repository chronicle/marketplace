...
from . import email_parser_decode, email_parser_regex
...
def noparenthesis(line: str) -> str:
    ...
    while True:
        lline = line_
        line_ = email_parser_regex.noparenthesis_regex.sub("", line_)
        if lline == line_:
            break
...
def cleanline(line: str) -> str:
    ...
    return email_parser_regex.cleanline_regex.sub("", line)
...
def get_domain_ip(line: str) -> list[str]:
    ...
    """
    m = (
        email_parser_regex.dom_regex.findall(" " + line)
        + email_parser_regex.ipv4_regex.findall(line)
        + email_parser_regex.ipv6_regex.findall(line)
    )
...
def parserouting(line: str) -> dict[str, typing.Any]:
    ...
    npline = npline.strip("\n")  # Remove any new-line
    raw_find_data = email_parser_regex.date_regex.findall(
        npline,
    ...
    if npdate:
        ...
        reg += email_parser_regex.escape_special_regex_chars.sub(r"""\\\1""", npdate)
    ...
    out["date"] = email_parser_decode.robust_string2date(npdate)
    ...
    if out.get("for"):
        ...
        m = email_parser_regex.email_regex.findall(out["for"])
        if m:
            out["for"] = list(set(m))
        else:
            del out["for"]
