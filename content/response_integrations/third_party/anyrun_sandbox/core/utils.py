...
def prepare_report_comment(results: list[dict]) -> str:
    ...
    return 'ANY.RUN Sandbox Indicators summary:\n' + raws if raws else 'Suspiciuos or Malicious indicators not found'
...
def prepare_base_params(siemplify) -> dict[str, str]:
    ...
    return {
        ...
        'opt_network_residential_proxy': extract_action_param(siemplify, param_name='opt_network_residential_proxy'),
        'opt_network_residential_proxy_geo': extract_action_param(siemplify, param_name='opt_network_residential_proxy_geo'),
...
def build_sandbox_indicators_payload(feeds: list[dict], task_uuid: str) -> list[dict[str, str]] | None:
    """
