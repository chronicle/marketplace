...
def initialize_lookup(
    ...
    with LookupConnector(
        ...
        report = connector.get_intelligence(lookup_depth=lookup_depth, **{lookup_entity: entity_identifier})
        
        siemplify.add_comment(f'Lookup url: {generate_lookup_reference(lookup_entity, entity_identifier)}')
...
def main():
    ...
    if query := siemplify.extract_action_param("query"):
        ...
        if not any([entity_identifiers, entity_types]):
            siemplify.end('At least one entity type and entity identifiers must be specified', False, EXECUTION_STATE_FAILED)
        ...
        for entity_type, entity_identifier in zip(entity_types, entity_identifiers):
            if lookup_entity := Config.ENTITIES.get(entity_type.lower()):
                verdict = initialize_lookup(siemplify, token, lookup_entity, entity_identifier, lookup_depth)
                siemplify.LOGGER.info(f'Entity: entity_identifier was lookuped. Json summary attached to the case wall.')
