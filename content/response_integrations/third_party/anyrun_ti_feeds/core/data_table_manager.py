...
class DataTableManager(object):
    ...
    def update_taxii_indicators(self, feed_fetch_depth: int) -> None:
        ...
        with FeedsConnector(
            ...
            for collection, data_table_name in Config.TAXII_DATATABLES.items():
                ...
                self._create_data_table(data_table_name, build_taxii_data_table_payload(data_table_name))
                self._retreive_indicators(connector, collection, data_table_name, feed_fetch_depth)

    def _is_datatable_exists(self, data_table_name: str,) -> None:
        """
    ...
    def _retreive_indicators(
        ...
        for feeds in FeedsIterator.taxii_stix(
            ...
            chunk_size=1000,
            limit=10000,
            modified_after=(datetime.now(UTC) - timedelta(days=feed_fetch_depth)).strftime(Config.DATE_TIME_FORMAT)
        ):
